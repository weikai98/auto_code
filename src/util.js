...
const util = {
    ...
    getProjectPath(document) {
        ...
        let workspaceFolders = vscode.workspace.workspaceFolders.map(item => item.uri._fsPath);
        ...
        workspaceFolders.forEach(folder => {
            ...
            if (currentFile.indexOf(folder) !== -1) {
                projectPath = folder;
            }
        })
        if (!projectPath) {
    ...
    getProjectRootPath(document) {
        ...
        return vscode.workspace.workspaceFolders.map(item => item.uri._fsPath);
    },
    /**
     * 获取当前工程名
     */
